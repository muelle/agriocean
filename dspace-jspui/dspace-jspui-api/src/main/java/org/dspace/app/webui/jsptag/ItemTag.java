...
/**
 * <P> JSP tag for displaying an item. </P> <P> The fields that are displayed
 * can be configured in
 * <code>dspace.cfg</code> using the
 * <code>webui.itemdisplay.(style)</code> property. The form is </P>
 *
 * <PRE>
 *
 ...
 * </PRE>
 *
 * <P> When using "resolver" in webui.itemdisplay to render identifiers as
 ...
 * already, it is simply rendered as a link with no other manipulation. </P>
 *
 * <PRE>
 *
 * <P>
 ...
 * </PRE>
 *
 * <P> You need to specify which strategy use for select the style for an item.
 * </P>
 *
 * <PRE>
 *
 ...
 * </PRE>
 *
 * <P> With the Collection strategy you can also specify which collections use
 * which views. </P>
 *
 * <PRE>
 *
 ...
 * </PRE>
 *
 * <P> FIXME: This should be more database-driven </P>
 *
 * <PRE>
 *
 ...
 * </PRE>
 *
 * <P> With the Metadata strategy you MUST specify which metadata use as name of
 * the style. </P>
 *
 * <PRE>
 *
...
public class ItemTag extends TagSupport {
                    ...
                    if (values[j] != null && values[j].value != null) {
                        ...
                        if (isLink) {
                            ...
                            if (values[j].authority != null
                                    && values[j].confidence >= MetadataAuthorityManager.getManager().getMinConfidence(values[j].schema, values[j].element, values[j].qualifier)) {
                                argument = "authority";
    ...
    private void renderFull() throws IOException, SQLException {
        ...
        if (ConfigurationManager.getBooleanProperty("google.plusone.enabled")) {
            out.println("<g:plusone size=\"small\"></g:plusone>");
        }
                    ...
                    for (int i = 0; i < bundles.length; i++) {
                        ...
                        for (int k = 0; k < bitstreams.length; k++) {
                            ...
                            if (!bitstreams[k].getFormat().isInternal()) {
                                ...
                                bsLink = bsLink
                                        + UIUtil.encodeBitstreamName(bitstreams[k].getName(),
    ...
    /**
     * Return the browse index related to the field.
     * <code>null</code> if the field is not a browse field (look for
     * <cod>webui.browse.link.<n></code> in dspace.cfg)
     *
     * @param field
