...
public class CitationManager {
        ...
        if (map.containsKey(tmpTMPL.definingField) && !map.get(tmpTMPL.definingField).equals("")) {
            ...
            if (tmpTMPL != null) {
                ...
                for (int i = 0; i < tmpTMPL.template.size(); i++) {
                    ...
                    while (m.find()) {
                        /*
                         * if metadata field is null - must skip it with related
                         * formatting. Also in the case of authors names we need
                         * put them all (we will have DCValue[] instead of just
                         * DCValue) in the map in that case
                         */
                        if ("bibliographicCitation.authors".equalsIgnoreCase(m.group(m.groupCount()))) {
                           ...
                           if (authors == null)
                            ...
                            if ((dcv != null) && (dcv.value != null)/*
                                     * map.containsKey(m.group(m.groupCount()))
                                     */) {
    ...
    public String updateCitationString(Item item) throws SQLException, AuthorizeException {
        ...
        DCValue oldCitations[] = item.getMetadata(MetadataSchema.DC_SCHEMA, "identifier", "citation", Item.ANY);
                ...
                if (this.types.size() > 0) {
                    ...
                    if (!citation.equals("")) {
                        if (oldCitations.length == 0) {
                            ...
                            //If identifier.citation already present we will not change it.
                            //It is users responsibility to review citation.
                            if (oldCitations.length > 0) {
                                return (oldCitations[0].value == null ? "" : oldCitations[0].value);
                            } else {
                                return "";
                            }
                        }
