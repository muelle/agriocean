 ...
 * items, as well as the editing of items further down the line. <p> Whenever
 * the submit servlet receives a GET request, this is taken to indicate the
 * start of a fresh new submission, where no collection has been selected, and
 * the submission process is started from scratch. <p> All other interactions
 * happen via POSTs. Part of the post will normally be a (hidden) "step"
 * parameter, which will correspond to the form that the user has just filled
 * out. If this is absent, step 0 (select collection) is assumed, meaning that
 * it's simple to place "Submit to this collection" buttons on collection home
 * pages. <p> According to the step number of the incoming form, the values
 * posted from the form are processed (using the process* methods), and the item
 * updated as appropriate. The servlet then forwards control of the request to
 * the appropriate JSP (from jsp/submit) to render the next stage of the process
 * or an error if appropriate. Each of these JSPs may require that attributes be
 ...
 * target collection. <p> When control of the request reaches a JSP, it is
 * assumed that all checks, interactions with the database and so on have been
 * performed and that all necessary information to render the form is in memory.
 * e.g. The SubmitFormInfo object passed in must be correctly filled out. Thus
 * the JSPs do no error or integrity checking; it is the servlet's
 * responsibility to ensure that everything is prepared. The servlet is fairly
 * diligent about ensuring integrity at each step. <p> Each step has an integer
 * constant defined below. The main sequence of the submission procedure always
 * runs from 0 upwards, until SUBMISSION_COMPLETE. Other, not-in-sequence steps
 * (such as the cancellation screen and the "previous version ID verification"
 * screen) have numbers much higher than SUBMISSION_COMPLETE. These conventions
 * allow the progress bar component of the submission forms to render the user's
...
public class SubmissionController extends DSpaceServlet {
    ...
    protected void doDSPost(Context context, HttpServletRequest request,
        ...
        } else // otherwise, figure out the next Step to call!
        {
            // Get the current step
            currentStepConfig = getCurrentStepConfig(request, subInfo);
     ...
     * @param stepConfig config of step who's page the user clicked "cancel" on.
     */
     ...
     * current request. If there is a problem,
     ...
     *
     * @param request HTTP request
     * @param si the current submission info
     *
     */
     ...
     *
     * @param request HTTP request
     * @param si The current SubmissionInfo object
     *
     ...
     *
     * @param request HTTP request
     * @param step The current SubmissionStepConfig
     */
     ...
     *
     * @param request HTTP request
     * @param beginningOfStep true if step just began
     */
     ...
     *
     * @param request HTTP request
     * @param cancellationInProgress true if cancellation is in progress
     */
     ...
     * @param subInfo the SubmissionInfo object pertaining to the current
     * submission
     * @param step the step to set as reached, can be also a previous reached
     * step
     * @param page the page (within the step) to set as reached, can be also a
     ...
     *
     * @throws ServletException if there are no more pages in this step
     */
