...
import org.dspace.submit.step.DescribeStepExt;
 ...
 */
public class JSPDescribeStepExt extends JSPStep
{
    ...
    /** Constructor */
    public JSPDescribeStepExt() throws ServletException
    {
            ...
            AuthorizeException
    {
        ...
        String buttonPressed = UIUtil.getSubmitButton(request, DescribeStepExt.NEXT_BUTTON);
        ...
        if (status == DescribeStepExt.STATUS_MORE_INPUT_REQUESTED)
        {
        ...
        else if (status == DescribeStepExt.STATUS_MISSING_REQUIRED_FIELDS)
        {
            List<String> missingFields = DescribeStepExt.getErrorFields(request);
            ...
            throws SQLException, ServletException, IOException
    {
        ...
        try
        {
            List<String> listoftypes = DescribeStepExt.getInputsReader(formFileName).getTypesListforCollection(c.getHandle());
            request.setAttribute("submission.types", listoftypes);
            String doctype = (String)request.getAttribute("submission.doctype");
            if(doctype==null)
            {
                DCValue[] values = subInfo.getSubmissionItem().getItem().getMetadata("dc", "type", null, Item.ANY);
                if (values.length > 0)
                {
                ...
                else
                {
                    if(listoftypes.size() > 0)
                        doctype = listoftypes.get(0);
                    else
                        doctype = ""; //nothing at all
                }
            }

            request.setAttribute("submission.inputs", DescribeStepExt.getInputsReader(formFileName).getInputs(c
                    .getHandle(), doctype));
        }
