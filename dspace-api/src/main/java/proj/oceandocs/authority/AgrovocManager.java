...
public class AgrovocManager implements ChoiceAuthority
{
//ICV
...
//DC
    ...
    public AgrovocManager()
    {
        if (!isConnected)
        {
            ...
            sql = ConfigurationManager.getProperty("dbagrovoc.sql");
...
//getMatches
    ...
    {
        int dflt = -1;
        List<String> agrovoc = new ArrayList<String>();
        Choice[] v;
        if (query.length() < 2)
        {
            v = new Choice[0];
        ...
        {
            agrovoc = db.getData(sql, query);
//        if (agrovoc.isEmpty())
//            agrovoc = db.getData(sql,"");
            v = new Choice[agrovoc.size()];

            for (int i = 0; i < agrovoc.size(); ++i)
            {
                String label = agrovoc.get(i);
...
//getLabel
    ...
    {
        System.out.println("(agrovoc)tokruju");
