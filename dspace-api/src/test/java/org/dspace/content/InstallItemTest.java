...
public class InstallItemTest extends AbstractUnitTest {
    ...
    public void testInstallItem_invalidHandle() throws Exception {
        ...
        new NonStrictExpectations() {
            ...
            {
                AuthorizeManager.authorizeActionBoolean((Context) any, (Community) any, Constants.ADD);
                result = false;
    ...
    public void testGetBitstreamProvenanceMessage() throws Exception {
        ...
        testMessage += "one: " + one.getSize() + " bytes, checksum: " + one.getChecksum() + " ("
                       + one.getChecksumAlgorithm() + ")\n";
        testMessage += "two: " + two.getSize() + " bytes, checksum: " + two.getChecksum() + " ("
                       + two.getChecksumAlgorithm() + ")\n";
        assertThat("testGetBitstreamProvenanceMessage 0", InstallItem.getBitstreamProvenanceMessage(item),
                   equalTo(testMessage));
    }
}
